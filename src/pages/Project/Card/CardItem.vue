<template>
  <div
    :class="`
            ${content_channging ? 'editting_card' : ''}
  ...
  >
    ...
    <q-card
      ...
      bordered
      flat
      class="cardBody overflow-hidden"
      :class="$q.dark.mode ? '' : 'border'"
    >
      <!-- 卡片顶部 -->
      <q-card-section
        v-if="!content_channging"
        class="row no-wrap q-pa-xs q-pr-sm border-bottom items-center hovered-item"
        :class="`
          ${
            (calc_auth('card', 'order', 'project') || isCreator) &&
            !name_changing
              ? 'dragBar'
              : ''
          }
      ...
      >
        ...
        <q-circular-progress
          v-else-if="
            cardRef.type === 'todo' && show_byPreference?.percent?.value
          "
          show-value
          :value="todo_process"
          :thickness="0.1"
        ...
        <q-btn
          ...
          size="sm"
          unelevated
          :icon="unfold_lessRef !== 'collapse' ? 'unfold_less' : 'unfold_more'"
          v-close-popup
          @click="unfold_less()"
          round
        ...
        <CardExecutor
          v-if="show_byPreference?.executor?.value"
          :card="cardRef"
          :executor="executor"
          :members="members"
          :isCreator="isCreator"
      ...
      <!-- 封面 -->
      <q-card-section
        v-if="default_version?.media?.url && unfold_lessRef !== 'collapse'"
        class="q-pa-xs"
      >
        <q-img
      ...
      <!-- 任务、备忘 -->
      <q-card-section
        v-if="cardRef.type === 'task' || cardRef.type === 'note'"
        class="column no-wrap card overflow-hidden"
        :class="`${content_channging ? 'no-padding' : 'q-py-sm q-px-md'}`"
        :style="unfold_lessRef === 'collapse' ? 'max-height: 6rem' : ''"
        @click="expandCard()"
      >
        <TipTap
        ...
        <TipTap
          ...
          class="undrag"
        />
        <!-- 宽度需要减去边框，否则会覆盖card的边框 -->
        <div
          v-if="unfold_lessRef === 'collapse'"
          class="absolute-bottom bg-gradient-bottom-unfold_card undrag"
          style="height: 60px; width: calc(100% - 2px)"
        ></div>
      </q-card-section>
      <!-- 待办 -->
      <q-card-section
        v-if="cardRef.type === 'todo'"
        class="column no-wrap q-px-none q-py-sm q-pb-sm overflow-hidden"
      ...
      >
        ...
        <div
          v-if="unfold_lessRef === 'collapse'"
          class="absolute-bottom bg-gradient-bottom-unfold_card"
          style="height: 60px; width: calc(100% - 2px)"
        ></div>
      </q-card-section>
      <!-- 卡片底部 -->
      <q-card-section
        v-if="unfold_lessRef === 'expand'"
        ...
        "
      >
        <template v-if="show_byPreference?.follow?.value">
          <overlappingAvatar
        ...
        <q-space />
        <div class="undrag flex flex-center hover-show transition">
          <q-btn
            flat
            dense
            size="sm"
...
.editting_card {
  border: 1px outset solid $q-primary;
  border-radius: 5px;
}
.highlight {
  width: 5px;
