<template>
  <div>
    ...
    <div
      v-if="columnRef && view_modelRef === 'kanban'"
      class="column no-wrap gap-xs items-center border-placeholder"
      :class="__dragging_column ? '' : 'full-height'"
      style="min-width: 322px"
    >
      <div
        class="full-width row no-wrap items-center q-px-xs q-pt-xs gap-xs radius-xs transparent font-medium"
      >
        <StatusMenu
      ...
      >
        ...
        <div
          v-if="!projectStore.cardDragging"
          data-dragscroll
          class="q-space"
    ...
    <div
      v-if="columnRef && view_modelRef === 'list'"
      class="column no-wrap gap-md items-center border-placeholder"
    >
      <div
      ...
      <q-markup-table flat bordered class="full-width q-space">
        <thead>
          <tr>
            ...
            <th class="text-left">清单</th>
            <th class="text-left">分值</th>
            <th class="text-left">进度</th>
            <th class="text-left">关注者</th>
...
import CardItem from "src/pages/Project/Card/CardItem.vue";
import RowCardItem from "src/pages/Project/Card/RowCardItem.vue";
    ...
    if (mm_wsStore.event && mm_wsStore.event.event == "posted") {
      ...
      if (strapi) {
        ...
        if (
          ...
          if (
            ...
            if (!card_users_ids?.includes(userStore.userId)) {
              ...
              if (projectStore.card.id === strapi.data.card_id) {
        ...
        if (
          ...
          if (res) {
            let card = res.data;
            if (projectStore.kanban.type === "kanban") {
