<template>
  <div class="q-space column no-wrap q-pb-lg">
    <template
      v-if="calc_auth(authBase?.collection, 'manageRole', authBase?.of)"
    >
      <q-toolbar class="transparent" :class="card && 'border-bottom'">
        <q-tabs v-model="active_role" dense>
          <q-tab
            v-for="i in role"
            :key="i.id"
            :name="i.subject"
...
import { __role } from "src/pages/Project/components/settings/projectRole_template.js";
import { __CardRole } from "src/pages/Project/components/settings/cardRole_template.js";
...
const processData = () => {
  ...
  if (projectStore?.card) {
    role.value = projectStore.card?.member_roles;
  } else if (projectStore?.project) {
    role.value = projectStore.project?.member_roles;
  }
};
...
watchEffect(() => {
  ...
  projectRole.value = projectStore.project?.member_roles;
  cardRole.value = projectStore.card?.member_roles;
});

watch(
  [projectRole, cardRole],
  () => {
    console.log("projectRole.value", projectRole.value);
    console.log("cardRole.value", cardRole.value);
...
const disable_checkbox_field = (collection, field) => {
  let disable_modify_fileds = ["author", "chat_service"];
...
const remove = async (member_role) => {
  ...
  if (res?.data) {
    ...
    if (isCardRef.value) {
      projectStore.card.member_roles = projectStore.card?.member_roles.filter(
        (i) => i.id != removed_id
      );
    } else {
      projectStore.project.member_roles =
        projectStore.project?.member_roles.filter((i) => i.id != removed_id);
    }
    init();
    isChanged.value = true;
  }
...
onBeforeUnmount(() => {
  if (isChanged.value) {
    // 手动更新数据 - 使用ws更新，这里注释
    // if (isCardRef.value) {
    //   projectStore.card.member_roles = projectStore.card?.member_roles?.map(
    //     (i) => (i.id === res.data.id ? res.data : i)
    //   );
    // } else {
    //   projectStore.project.member_roles =
    //     projectStore.project?.member_roles?.map((i) =>
    ...
    if (isCardRef.value) {
      MsgParmas.is = "card";
      MsgParmas.card_id = projectStore.card.id;
    } else {
      MsgParmas.is = "project";
      MsgParmas.project_id = projectStore.project.id;
    }
    let chat_Msg = {
      body: `${userStore.me?.username}更新了${
        projectStore.card ? "卡片" : "项目"
      }的权限系统`,
      props: {
        strapi: {
          data: MsgParmas,
...
const createRole = async () => {
  ...
  if (isCardRef.value) {
    createRole_params.value.by_card = projectStore.card?.id;
  } else {
    createRole_params.value.by_project = projectStore.project?.id;
  }
  const res = await createMemberRole(createRole_params.value);
  if (res?.data) {
    // if (isCardRef.value) {
    //   projectStore.card.member_roles.push(res?.data);
    // } else {
    //   projectStore.project.member_roles.push(res?.data);
    // }
