...
import ScheduleBody from "src/pages/Project/schedule/ScheduleBody.vue";
...
const { proj_id: project_id, schedule_id, by } = toRefs(props);
...
const init = () => {
  if (by.value === "project") {
    schedules.value = projectStore.project?.schedules;
    schedule.value = schedules.value?.find(
      (i) => i.id == Number(schedule_id.value)
    );
  }
  if (by.value === "card") {
    schedule.value = projectStore.card?.schedule;
  }
  if (by.value === "user") {
  }
  // console.log('init');
...
const set_store = (schedule) => {
  projectStore.project = {
    ...projectStore.project,
    schedules: projectStore.project.schedules.map((i) =>
...
const process_createData = (val) => {
  ...
  if (by.value === "card") {
    ...
    projectStore.card = {
      ...projectStore.card,
      schedule: {
        ...projectStore.card.schedule,
        schedule_events: [
          ...projectStore.card.schedule.schedule_events,
          ...val,
        ],
      },
    };
