  ...
  <div class="absolute-full" :class="`drop-zone_${storage_idRef}`">
    <drag-select v-model="selected">
      <q-scroll-area
        ...
        :class="`drop-zone_${storage_idRef}`"
        @mouseenter="projectStore.active_folder = storage_idRef"
        @mouseleave="showUpdateArea = false"
        @click="clickColumn()"
      >
        <!-- <ul>
          <li>{{ sub_foldersRef }}</li>
          <li>{{ sub_filesRef }}</li>
        </ul> -->
        <q-list v-if="masterList" dense class="q-pa-xs">
          <slot
            v-if="projectStore.create_folder_ing"
            name="create_folder"
          ></slot>
                    ...
                    <q-item-section>
                      <div class="row no-wrap gap-sm">
                        <q-icon
                          ...
                          dense
                          :size="
                            element?.color_marker === marker ? '24px' : '16px'
                          "
                          flat
                          round
                          padding="none"
...
const isActivedFolder = computed(
  () => projectStore.active_folder === storage_idRef.value
);
...
watchEffect(() => {
  disable_dragSelect.value = projectStore.create_folder_ing;
  masterList.list = [...sub_foldersRef.value, ...sub_filesRef.value];
});
// const process_masterList = () => {
//     masterList.list = [...sub_foldersRef.value, ...sub_filesRef.value];
// }
// onMounted(() => {
//     process_masterList()
// })
// watch([sub_filesRef,sub_foldersRef],() => {
//     console.log('process_masterList');
//     process_masterList()
...
watchEffect(() => {
  ...
  if (batch_selected_ids.value.length > 0) {
    ...
    projectStore.selected = selected.value;
  }
});
      ...
      if (batchCreated?.data) {
        ...
        if (!storage_id) {
          if (belonged.value === "card") {
            // todo 需要补充card名称
            chat_Msg.body = `${userStore.me?.username}向卡片${projectStore.card.id}上传了文件`;
...
const createStorageFn = async (folder, storage_id) => {
  ...
  if (res) {
    ...
    if (belonged.value === "card") {
      // todo 需要补充文件夹名称
      chat_Msg.body = `${userStore.me?.username}在卡片 - ${projectStore.card.name} 内新建了文件夹：`;
      chat_Msg.props.strapi.data.card_id = projectStore.card.id;
    } else if (belonged.value === "project") {
      // todo 需要补充文件夹名称
...
const removeFile = async (file) => {
  ...
  if (remove) {
    let chat_Msg = {};
    if (belonged.value === "card") {
      chat_Msg.body = `${userStore.me?.username}删除了卡片 - ${projectStore.card.name} 内的文件 ${file.name}`;
    } else if (belonged.value === "project") {
...
const removeFolder = async (folder) => {
  ...
  if (res_remove) {
    ...
    if (belonged.value === "card") {
      chat_Msg.body = `${userStore.me?.username}删除了卡片 - ${projectStore.card.name} 内的文件夹 ${folder.name}`;
      chat_Msg.props.strapi.data.card_id = projectStore.card.id;
    } else if (belonged.value === "project") {
...
const handleDragLeave = (e) => {
  ...
  if (e.target == dropZone.value) {
    ...
    // 设置活动文件夹为null
    projectStore.active_folder = null;
  }
};
