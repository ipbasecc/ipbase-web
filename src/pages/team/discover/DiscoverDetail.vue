<template>
    <q-layout v-if="card" view="hhh LpR fFf" container class="absolute-full"
    @mousemove="handleMouseMove" @mouseup="handleMouseUp">
        <q-drawer v-if="!need_buy"
            v-model="rightDrawerOpen" side="right" bordered class="column"
                ...
                <template v-if="current_classExtend === 'class_overview'">
                    <KeepAlive>
                        <OverView wasAttached_to="card" class="absolute-full"
                            :hideMedia="true" @sync_version="sync_version"
                        />
                    </KeepAlive>
                </template>
                        ...
                        <q-btn dense flat padding="xs md" :label="version_label(activeVersion?.name)">
                            <q-menu class='radius-sm'>
                                <q-list bordered dense class='radius-sm q-pa-xs' style="min-width: 100px">
                                    <q-item v-for="i in overviews" :key="i.id"
...
<script setup>
import { ref, onMounted, computed, onUnmounted,watchEffect,reactive } from 'vue'
...
onMounted(async () => {
    await queryOneDiscoverFn(activeCard.id)
})
onUnmounted(() => {
    teamStore.card = null
})
