...
const fetch_thread_Msgs_options = ref(
  `fromPost=${fromPost.value}&fromCreateAt=${fromCreateAt.value}`
);
const hasMore_thread_Msgs = ref();

const fetch_thread_Msgs = async () => {
  let res = await getThread(
    thread_post_idRef.value,
    fetch_thread_Msgs_options.value
  );
