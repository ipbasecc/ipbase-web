// This is just an example,
// so you can safely delete all default props below

export default {
  App_Lang: 'zh-CN', //用于页面切换语言时，页面重新获取语言，不要直接翻译，应该直接设置对应语言代号
  app_name: "易乎APP",
  Language: "语言",
  app_loading: "IPBase",
  app_loading_tip: "每个人都有自己的品牌",
  upload_file: "文件上传",
  login: "登录",
  please_enter_account_password: "请输入账号密码",
  no_account: "没有账号？点此注册",
  forgot_password: "忘记密码？",
  set_server_address: "设置服务器地址",
  login_failed: "登录失败",
  invalid_identifier_or_password: "无效的账号或密码",
  authorization_verification: "授权校验：",
  verifying: "验证中",
  verification_completed: "验证完成",
  verified: "已认证",
  unverified: "未认证",
  logging_in: "登录中",
  login_success: "登录成功",
  welcome_back: "欢迎回来",
  auto_redirect: "将在",
  seconds: "秒后自动跳转",
  redirect_now: "立即跳转",
  register: "注册",
  user_agreement: "服务协议",
  agree: "同意",
  agree_user_agreement_and_register: "同意服务协议并注册",
  username: "用户名",
  email: "邮箱",
  invalid_email: "邮箱输入有误，请检查输入！",
  password: "密码",
  confirm_password: "确认密码",
  password_not_match: "两次输入的密码不一致",
  have_account: "已有账号？",
  login_now: "立即登录",
  please_login: "请先登录",
  user_created_successfully_logging_in: "用户创建成功，正在登录，请稍后...",
  login_success: "登录成功！",
  register_success: "注册成功！",
  welcome_to_join: "欢迎加入，",
  switch_language: "切换语言",
  auto_redirect: "将在",
  seconds: "秒后自动跳转",
  redirect_now: "立即跳转",
  data_security: "数据安全：第三方无法访问自有数据；",
  high_performance: "高性能：本地设备性能远胜于云服务器，内网使用自建服务器性能强悍。",
  set_your_server: "自建服务器？",
  set_your_server_description: "您可以在本地、内网、公网假设自己的服务器，请参考",
  server_deployment_description: "中服务端部署部分",
  set_your_server_benefits: "自建服务器的好处？",
  official_server: "官方服务器",
  what_is_server: "什么是服务器？",
  server_description: "服务器是指为软件正常运行提供数据存取、用户管理、实时通讯等功能的支持服务端应用。",
  server_deployment_description: "中服务端部署部分",
  current_server: "当前服务器",
  clear_cache: "清除缓存",
  this_demo: "此演示",
  upload: "上传",
  download: "下载",
  close: '关闭',
  channel_nav_posts: "动态",
  channel_nav_articles: "文章",
  channel_nav_videos: "视频",
  channel_nav_audios: "音频",
  channel_nav_chat: "聊天",
  channel_nav_sales: "橱窗",
  app_update_tip: "应用有新更新可用，请保存您的工作并刷新页面。",
  refresh_page: "刷新页面",
  public_channel: "公共频道",
  private_channel: "私有频道",
  edit_channel: "编辑频道属性",
  invite: "邀请",
  invite_member: "邀请成员",
  remove_channel: "删除频道",
  create_channel: "新建频道",
  channel_name: "频道名称",
  channel_purpose: "频道说明",
  team: "团队",
  overview: "概览",
  navigation_threads: "讨论主题",
  navigation_Quadrant: "事务沙盘",
  navigation_News: "团队资讯",
  navigation_Orders: "我的购买",
  channel: "频道",
  project: "项目",
  community_channel: "社群频道",
  collaboration_project: "协作与服务",
  video: "视频",
  create_project: "新建项目",
  project_private: '仅成员可见',
  shortcut_table_header_desc: "功能",
  shortcut_table_header_key: "快捷键",
  shortcut_table_header_props: "可用条件",
  shortcut_expand_next_nav: "展开下级导航",
  shortcut_expand_next_nav_props: "存在下级导航且其已显示",
  shortcut_collapse_next_nav: "折叠下级导航",
  shortcut_collapse_next_nav_props: "存在下级导航且其未显示",
  hide_atom_chat: "隐藏快捷沟通面板",
  hide_atom_chat_props: "快捷沟通开启并处在聚焦状态",
  quick_create_card: "快速创建卡片",
  quick_create_card_shortcut: "分栏空白处双击",
  quick_create_card_props: "",
  cancel_quick_create_card: "取消创建卡片",
  cancel_quick_create_card_props: "正在创建卡片",

  save: "保存",
  saving: "保存中...",
  connect_refused_header: "连接被拒绝！",
  connect_refused_caption: "服务器可能离线，请返回登陆页面，修改服务器地址后重新连接",
  connect_refused_btn_label: "重新登陆",
  connect_refused_btn_pageRefresh_label: "刷新重试",
  connect_refused_login_caption: "服务器可能离线，修改服务器地址后重新连接",
  connect_refused_login_btn_label: "修改服务器地址",

  updateUserTodogroups_error_feedback: "更新分组数据时发生错误，请刷新页面",
  refresh: '刷新',

  need_pagination_info: '分页信息不能缺少',
  update_error_need_refresh: '更新卡片数据时发生错误，请刷新页面',

  oss_premission_error: "获取文件访问权限出错",
  upload_error: '上传错误',
  cant_preview_need_download: '当前文件不能预览，请下载后查看',
  clean_all: '清除全部',
  clean_all_complate: '清除完成',
  upload_tip_rBtn: '点击右侧按钮、或拖拽图片到此处上传预览图',
  support: '支持',
  pick_file: '选择文件',
  upload_complated: '上传完成',
  upload_pick_or_drop_tip: '点击按钮选择文件，或拖拽文件到下方',
  by_time: '于',
  get_file_error: '文件获取错误',
  clean_color: '清除颜色',
  start_type_tip: '在此开始你的输入 …',

  user_center: '用户中心',
  my_channel: '我的频道',
  user_channel: '用户频道',
  user_settings: '用户设置',
  logout: '注销登录',
  logout_confirm: '确认注销',
  login: '登录',
  regist: '注册',

  status_online: '在线',
  status_away: '离开',
  status_dnd: '勿扰',
  status_offline: '离线',
  clear_all: '清除全部',
  read_all: '全部已读',

  app_team_purpose: '管理你的团队、频道、项目',
  affairs: '事务',
  app_affairs_purpose: '管理个人事务',
  brand: '品牌',
  app_brand_purpose: '打造你的个人、团队品牌',
  threads: '主题',
  app_threads_purpose: '访问你参与讨论的主题',
  chats: '沟通',
  app_chat_purpose: '与您的好友对话',
  deliver: '交付',
  app_deliver_purpose: '发布你的需求，交付你的产品',
  aichat: 'AI对话',
  app_aichat_purpose: '与AI对话',

  deal_jsonContent: '说明',
  deal_todogroups: '待办',
  deal_party_a_requirements: '甲方附加要求',
  deal_party_b_requirements: '乙方附加要求',
  deal_deliver: '交付',
  deal_homepage: '交付首页',
  deal_party_a_list: '找活干',
  deal_party_b_list: '找人干活',
  deal_no_listdata: '暂无数据',
  deal_my_party_a_list: '我的需求',
  deal_my_party_b_list: '我的承接',
  deal_my_talkers: '我的沟通',
  to_certification: '去认证',
  deliver_talkers_panel: '交付：沟通面板',
  aichat_panel: 'AI对话',

  deal_status_awaiting_deposit: '待预存',
  deal_status_deposited: '已预存',
  deal_status_pending_matching: '待对接',
  deal_status_matching: '对接中',
  deal_status_progressing: '执行中',
  deal_status_reviewing: '审核中',
  deal_status_completed: '已完成',
  deal_status_withdrawn: '已提款',

  manual: '操作手册',
  shortcut: '快捷键',

  ipbase_brand: 'IPBase',
  ipbase_brand_purpose: '每个人都有自己的品牌',

  gohome: '回首页',
  go_teamhome_label: '团队协作',
  go_teamhome_caption: '组织你的团队，开展业务或完成项目。',

  document_download: '文档下载：',
  click_or_drag_to_upload: '点击或拖拽文件到此处上传',
  upload_not_support_mutiple: '不支持多文件上传',

  edit: '编辑',
  no_video: '没有视频',
  no_media: '没有媒体',
  no_cover: '没有封面',
  unread: '未读',
  new_tag: '新标签',
  user_have_no_channel: '该用户没有创建自己的频道',
  speed: '速度',
  creator: '创作者',
  visit_channel: '访问他的主页',
  have_no_descripiton: '未补充说明信息',
  have_no_responsibility: '未补充负责内容信息',
  user_blocked: '该用户已被封禁！',

  owner: '所有者',
  executor: '负责人',
  send_message: '发送消息',
  _with: "与",
  s_talk: '的对话',
  all_cards_loaded: '已完成全部卡片加载',

  hide_completed_todo: '隐藏已完成待办',
  focus_model: '专注模式',
  enter_functions: '进入相应功能',
  click_right_navigation: '请点击右侧导航内容',
  related: '相关',
  followed: '关注',
  my_executed: '我负责的',
  my_related: '与我相关',
  my_followed: '我关心的',
  ws_error_need_refresh_tip: 'WebSocket 无法连接，部分功能可能无法使用，如果发现功能没有正常执行，可以在操作后手动刷新页面',

  expand: '展开',
  collapse: '折叠',
  card: '卡片',
  followed_card: '关注了此卡片',
  followe_this_card: '关注了此卡片',
  view_detial: '查看详情',
  task_detial: '任务详情',
  course_detial: '课程详情',
  resource_detial: '资源详情',
  rename_card: '重命名卡片',
  edit_course: '编辑课程',
  edit_resource: '编辑资源',
  change_to: '修改为：',
  status: '状态',
  delete: '删除',
  delete_card: '删除卡片',
  delete_card_segment: '删除镜头',
  class_card: '课程',
  resource_card: '资源',
  is_creator_tip: '您是资源拥有者，无需购买',
  already_buy_tip: '您已购买此资源',
  already_buy_auth_tip: '您已购买此授权',
  is_not_buyer_tip: '您尚未购买该资源，无权下载此内容。',
  task_card: '卡片',
  show_more: '显示更多',
  todo: '待办',
  task: '任务',
  document: '文档',
  file: '文件',
  storage: '存储',
  schedule: '规划',
  feedback: '反馈',
  note: '笔记',
  chat: '讨论',
  pendding: '待处理',

  quick_chat: '快速沟通',
  double_click_to_close: '双击关闭',
  assign_member: '指定成员',
  assign_executor: '指定负责人',
  remove_executor: '指定负责人',
  no_premission_to_view: '您无权查看此内容',
  no_premission_perform_operation: '您无权执行此操作',
  no_premission_to_edit: '您无权编辑此内容',
  no_premission_to_delete: '您无权删除此内容',
  no_premission_to_comment: '您无权评论此内容',
  no_premission_to_follow: '您无权关注此内容',
  no_premission_to_unfollow: '您无权取消关注此内容',
  no_premission_to_assign: '您无权指定负责人',
  no_premission_to_unassign: '您无权取消指定负责人',

  kanban: '看板',
  list: '列表',
  quadrant: '四象限',
  quadranta: '重要 · 不紧急',
  quadrantb: '重要 · 紧急',
  quadrantc: '不重要 · 不紧急',
  quadrantd: '不重要 · 紧急',
  score: '分值',
  role: '权限',
  private_setting: '可见性',
  public: '公开',
  private: '私有',

  please_fill_code: '请输入提取码：',
  sharlink_error_tip: '分享链接有误，请检查链接是否正确!',
  share_code: '提取码：',

  settings: '设置',
  type_name_here: '在此输入名称…',
  type_price_here: '在此输入价格…',
  cancel: '取消',
  create: '创建',
  no_subtask: '尚无任何子任务',
  no_todos: '尚无任何待办事项',

  clean_cache: '清除缓存',
  clean: '清除',
  pinned_messages: '置顶的消息',
  favorited_message: '收藏的消息',
  chose_a_channel: '请先选择一个聊天频道',
  no_more_message: '没有更多消息了',
  no_favorited_message: '没有更多消息了',
  got_new_messages: '收到新消息',
  view_now: '立即查看',
  load_more: '加载更多...',

  all_members_group: '全部成员',
  you: '你',
  pinned: '已置顶',
  favorited: '已收藏',
  reply: '回复',
  follow_thread: '关注主题',
  favorite_message: '收藏消息',
  unfavorite_message: '取消收藏',
  pin_message: '置顶消息',
  unpin_message: '取消置顶',
  show_unread_history_message: '显示未读历史消息',
  AM: '上午',
  PM: '下午',

  create_chat_channel: '新建讨论频道',
  project_overview: '项目概览',
  project_title: '项目名称',
  project_title_input_tip: '请在此处输入您的项目名称',
  project_title_rules_tip: '最多32个字符，请勿包含特殊字符',
  project_description: '摘要、备注、说明',
  project_description_input_tip: '您可以为您的项目添加简单的说明，方便成员理解项目内容',
  project_description_rules_tip: '项目名称最多256个英文字符、或128个中文',
  creating_waiting: '正在创建，请稍候…',

  public_project: '公开项目',
  private_project: '私有项目',

  share_this: '分享此',
  have_shared_this: '已分享此',
  can_share_or_edit: '您可以直接分享此连接，或修改共享',
  back: '返回',
  motify: '修改',
  delete_and_reshare: '删除并创建新共享',
  share_expired_reshare: '该共享已失效，如有需要，请点击修改按钮更新共享连接',
  new_share_cancel_old_still: '您并没有新建“共享”，旧“共享”将依然存在',
  known_it: '知道了',
  cannot_modify_sharecode: '共享已发布，不可以修改共享码',
  or_others_cannot_visit: '否则其它用户将不能使用其访问',
  you_can_delete_this_share: '您可以删除此共享',
  you_can_recreate_share_after_delete: '您可以在删除后重新共享，从而废弃旧共享，使用新共享',

  view_limit: '最大查看次数',
  view_limit_rule_tip: '不能小于0',
  date_limit: '有效期',
  chose_share_moudle: '允许查看关联的：',
  available_times: '可用次数',
  expried_data: '过期时间',
  forever: '永久',
  url_copy_secuss: '邀请链接已复制到剪贴板',
  copy_error: '复制出错，请手动复制',
  create_team: '创建团队',
  team_name: '团队名称',
  team_introduce: '团队介绍',
  operation_mode: '运营模式',
  update_channel: '更新频道',
  update: '更新',
  double_click_change_name: '双击改名',
  change_version_name: '修改版本名称',
  version_name: '版本名称',
  add_version: '添加新版本',
  remove_this_version: '移除此版本',
  default_version: '默认版本',
  'Default version': '默认版本',
  classroom: '课堂',
  resource: '资源',
  segment: '镜头',
  ctrl_enter_send: 'Ctrl + Enter 发送',
  channel_not_join_or_was_blocked: '您尚未正式加入该频道、或您已被该项目管理员屏蔽',
  project_not_join_or_was_blocked: '您尚未正式加入该项目、或您已被该项目管理员屏蔽',
  project_is_unconfirmed: '您已加入，请等待管理员审核。',
  project_is_not_member: '您尚未加入该项目',
  project_is_blocked: '您已被该项目管理员屏蔽',
  no_verifild_cant_access: '您尚未通过管理员验证，暂时不能访问此团队',
  was_blocked_cant_access: '您已被该团队管理员屏蔽，请尝试联系管理员申诉',

  manage_member: '成员管理',
  team_settings: '团队设置',
  leave_team: '离开团队',
  delete_team: '删除团队',
  about_team: '关于团队',

  cannt_do_it: '不能执行此操作',
  doing_wait: '正在执行操作，请稍候...',

  team_icon: '团队标志',
  update_team_icon: '修改标志',

  view_all_threads: '查看全部主题',
  have: '有',
  items_unread: '条未读',
  no_unread_thread: '没有未读主题',
  items_talk_message: '条讨论',
  send_to_self: '给自己留言',

  theme: '主题',
  profile: '个人资料',
  others: '其它',

  update_password: '修改密码',
  old_password: '旧密码',
  new_password: '新密码',
  confirm_password: '确认密码',
  password_not_same: '新密码与确认密码不一致',
  password_is_same: '新密码与旧密码相同，没有必要更新',
  password_cant_same: '新密码不能与旧密码相同',
  change_success: '修改成功',

  dark_mode: '深色',
  light_mode: '亮色',

  examine: '审核',
  doubel_click_add_desc: '双击此处添加概述',

  chang_project_desc_to: '将项目摘要修改为：',
  chang_card_desc_to: '将项目摘要修改为：',
  chang_project_title_to: '将项目标题修改为：',
  chang_card_title_to: '将卡片摘要修改为：',

  end: '结束',
  start: '开始',
  change_overview_preview: '修改概览图',
  remove_overview_preview: '移除概览图',

  classroom_file: '课程文件',
  resource_file: '资源文件',
  preview_file: '预览文件',
  sale_content: '销售内容',
  sale_purpose: '销售目的',
  sale_caption: '销售说明',
  agree_service_argment: '同意服务条款',
  auth_extened_label: '附加授权说明',
  price_0_tip: '存在0元授权，是否继续？',
  name_required: '请输入名称',
  sale_content_required: '请输入销售内容',
  agree_service_argment_required: '发布前必须同意服务条款',
  resource_type_required: '请选择资源类型',
  resource_type: '资源类型',
  article: '文章',
  video: '视频',
  audio: '音频',
  file: '文件',
  resource_article_description: '图文资料内容',
  resource_video_description: '拍摄视频、动画、短视频等',
  resource_audio_description: '音乐、音频、音效等作品',
  resource_file_description: '源文件、素材、模板等',
  exclusive_caption: '商品被购买后，您将无权在其他渠道销售，购买者将获得商品的独家使用权；同时该商品将不再显示在您的销售列表中',
  commercial_caption: '购买者被允许在商业场景下使用该产品，允许其用于二次生产、销售、服务',
  'non-commercia_caption': '购买者被允许在非商业场景下使用该产品，允许其用于个人学习、研究、欣赏等非商业性用途',
  educational_caption: '购买者被允许在教育场景下使用该产品，允许其用于教学、研究、欣赏等教育性用途',
  personal_caption: '购买者被允许在个人场景下使用该产品，允许其用于个人学习、研究、欣赏等非商业性用途',
  doubel_click_rename: '双击改名',
  add_version_failed: '新增失败',
  default_version_name: '默认版本',
  invite_member_to_project: '邀请成员加入项目：',
  chose_a_available_data: '请选择一个有效期',

  invite_message: '留言',
  i_am: '我是',
  invite_you_to_team: '邀请你加入团队:',
  waitting_for_you: '等你哦！',
  one_day: '一天',
  one_week: '一周',
  one_month: '一个月',
  one_year: '一年',
  need_limit_times_or_date: '最大可用次数 与 有效期，不能同时都不限制！',

  create_document: '新建文档',
  create_title: '文档标题',

  changed_project_document: '修改了项目文档',
  changed_task_document: '修改了任务文档',
  sorted_project_document: '排序了项目文档',
  sorted_task_document: '排序了项目文档',
  deleted_project_document: '删除了项目文档',
  deleted_task_document: '删除了项目文档',

  changed_project_props: '修改了项目：',
  changed_card_props: '修改了卡片：',

  inner_document: '内的文档：',
  of_content: '的内容：',

  create_chat_channel_fiald_try_refresh: '创建通讯频道失败，请刷新页面后重试',
  ws_offline_tip: '实时消息服务离线，其他人需要刷新页面才能看到你的更新',

  remove_this_group: '移除此分组',
  new_kanban: '新看板',
  new_reel: '新轨',
  new_classroom: '新课堂',
  new_resource: '新资源',
  kanban_name: '看板名称',
  new_group: '新分组',
  group_name: '分组名称',
  new_group_name: '新分组名称',
  click_btn_above: '请点击上方按钮新建',
  click_btn_bellow: '请点击下方按钮新建分组',

  sort_kanban: '调整了看板分组的排序',
  cant_delete_include_kanban: '不能删除包含看板的分组',
  name: '名称',
  remove: '移除',
  new: '新',

  workspace_kanban: '工作空间',
  workspace_classroom: '课程',
  workspace_segment: '桌面',
  workspace_resource: '资源集合',

  column: '分栏',
  Reel: 'Reel',
  Chapter: '章节',
  column_name: '分栏名称',
  default_create: '默认创建',
  display_settings: '显示设置',
  delete_column: '删除分栏',
  create_card: '新建卡片',
  thumbnial: '预览图',
  title: '名称',
  content: '内容',
  progress: '进度',
  follower: '关注者',
  more_actions: '操作',

  attention: '注意！',
  delete_column_include_cards_tip: '分栏内包含有卡片，如果删除分栏，卡片也会被清理，是否继续？',
  delete_reel_include_cards_tip: 'Reel内包含有卡片，如果删除分栏，卡片也会被清理，是否继续？',
  cant_view_private_card_tip: '该卡片已被设置为私有，只有卡片成员可查看其内容',

  remove_this_kanban: '移除此看板',
  remove_this_chapter: '移除此章节',
  remove_this_reel: '移除此轨',
  add_new: '添加新',
  miss: '缺少',
  not_support_upload_dir: '此处不支持上传文件夹',
  schedula_desc_placeholder: '在此输入该“规划”的摘要',
  share: '共享',
  share_expried: '共享已失效',
  schedule_name: '规划名称',
  create_schedule: '新建规划',
  schedule_tip_event: '常规',
  schedule_tip_handshake: '多方协作',
  schedule_tip_rocket: '紧张进行中...',
  schedule_tip_busy: '疯狂赶工中...',

  share_your_schedule: '共享你的规划',
  is_shared_can_share_or_update: '你已共享过该规划,您可以直接分享此连接，或修改共享',

  no_premission_to_invite: '没有权限邀请成员',
  remove_member: '移除成员',

  archive_project: '归档项目',
  delete_project: '删除项目',

  card_preference_settings: '卡片偏好设置',
  project_preference_settings: '项目偏好设置',
  kanban_preference_settings: '看板偏好设置',
  project_settings: '项目设置',
  please_wait: '请稍候...',
  project_setting_basic: '基本信息',
  project_service_content: '服务信息',
  project_setting_enable: '功能设置',
  project_setting_preferences: '偏好设置',
  project_setting_role: '权限',
  project_setting_members: '成员',
  project_setting_more: '更多',

  server_no_custom_tip: '尚未添加自定义服务器,正在使用官方服务器',
  server_add_tip: '点击下方按钮，添加自定义服务器',
  add_server: '新增服务器',
  server_name: '服务器名称',
  server_url: '服务器备注',
  server_note: '服务器备注',

  server_unvailable: '服务器不可用',
  server_url_error_tip: '服务器地址格式错误：必须是 http:// 或 https:// 开头的有效地址',

  folder_name: '文件夹名称',
  new_folder: '新建文件夹',

  drag_here_upload: '拖拽文件到此处上传',
  rename: '重命名',
  create_storage: '创建存储',
  default_storage: '默认存储',
  storage_name: '存储名称',
  fill_azure_info: '请填写Azure Blob Storage信息',
  attach_file: '附加文件',
  join_talk: '留言讨论',

  todo_attach_txt_private_kanban: '关联个人',
  todo_attach_txt_person: '个人',

  add_todogroup: '添加待办分组',
  add_todo: '添加待办',
  todogroup_name: '待办分组名称',
  rf_delete_todogroup: '删除该分组及其所有待办？',
  delete_todogroup: '删除该分组',
  todo_content: '待办内容',
  group: '分组',

  kanban_attach_todo_tip_lineA: '如果你正在处理此看板中的任务',
  kanban_attach_todo_tip_lineB: '如果你需要针对此看板添加一些备忘或待办',
  kanban_attach_todo_tip_lineC: '如果这些备忘或待办只是写给自己的',
  kanban_attach_todo_tip_lineD: '那么请在此处操作',
  kanban_attach_todo_tip_lineE: '您可在此记录自己需要处理的事项',
  kanban_attach_todo_tip_lineF: '此为全局备忘，可以不关联任何内容',
  kanban_attach_todo_tip_lineG: '此处内容仅您个人可见',

  card_settings_status_label: '状态',
  card_settings_status_desc: '显示和设置卡片状态：待处理、处理中、遇阻、待审核、已完成',
  card_settings_score_label: '分值',
  card_settings_score_desc: '管理成员可以为每项任务指定分值，以便后期统计每个成员对项目的共享，关闭后该功能不可用',
  card_settings_percent_label: '进度',
  card_settings_percent_desc: '如果卡片是待办类型，那么会在左上角显示已完成待办的进度，你可以在此控制该内容是否显示在卡片上',
  card_settings_executor_label: '负责人',
  card_settings_executor_desc: '显示和设置卡片的“负责人”，禁用则不会在卡片上显示',
  card_settings_follow_label: '关注',
  card_settings_follow_desc: '显示和设置卡片的“关注”功能，禁用后“关注卡片”功能不可见',
  card_settings_color_marker_label: '颜色标签',
  card_settings_color_marker_desc: '卡片右下角会显示颜色指示灯，仅用该功能将不可用',
  project_settings_allow_join_requests_label: '允许申请加入',
  project_settings_allow_join_requests_desc: '允许成员申请加入，禁用后，成员只能通过邀请连接加入',
  project_settings_allow_direct_chat_label: '允许私信',
  project_settings_allow_direct_chat_desc: '允许成员之间直接发送私信，禁用后如果成员间需要发送私信，需互为好友', 
  project_settings_chat_mode_label: '沟通模式',
  project_settings_chat_mode_desc: '选择沟通模式，支持“社群讨论”和“信息服务”两种模式',
  project_settings_chat_mode_chat_label: '社群讨论',
  project_settings_chat_mode_info_label: '信息服务',
  project_settings_chat_mode_chat_desc: '已讨论的形式提供信息服务，允许成员发送消息。',
  project_settings_chat_mode_info_desc: '信息由运营人员发送，成员可以查看，但无法发送消息。',

  enable_settings_multiple_boards_label: '工作空间',
  enable_settings_multiple_boards_desc: '允许创建多个工作空间，以便在同一个项目中，启用多个任务管理应用',
  enable_settings_multiple_boards_locked_tip: '该项目已经存在多个工作空间，不能禁用该功能，否则除了第一个工作空间外，其它均不能正常访问',
  enable_settings_chat_label: '讨论',
  enable_settings_chat_description: '讨论模块，由全体项目成员组成的群聊，您也可以发起一对一沟通',
  enable_settings_kanban_label: '看板',
  enable_settings_kanban_description: '展示项目卡片，核心功能，建议开启',
  enable_settings_classroom_label: '课堂',
  enable_settings_classroom_description: '为“课堂”场景定制的功能模块，您可以以看板的形式展示和管理课程资源',
  enable_settings_resource_label: '资源',
  enable_settings_resource_description: '在此发布、销售视频、音频、图像、文档等资源，为成员提供资源服务',
  enable_settings_segment_label: '镜头',
  enable_settings_segment_description: '参考Autodesk Flame的界面，为后期制作团队提供的“镜头制作”管理功能',
  enable_settings_document_label: '文档',
  enable_settings_document_description: '基于Markdown编辑器的文档功能，您可以在此简单共享文档',
  enable_settings_storage_label: '文件',
  enable_settings_storage_description: '内置的简易“网盘”，您可以使用我们内置的基于阿里云OSS的存储，也可以使用您自己的Azure',
  enable_settings_schedule_label: '规划',
  enable_settings_schedule_description: '简易日程管理工具，您可以创建多个规划日历，并设置其状态，或者共享给外部人员',
  enable_settings_budget_label: '预算管理',
  enable_settings_budget_description: '记录项目执行过程中的收支，方便进行项目成本核算',

  field_creator: '创建者',
  field_owner: '所有者',
  field_admin: '管理员',
  field_blocked: '被屏蔽',
  field_reviewer: '审核者',
  field_executor: '负责人',
  field_member: '成员',
  field_staff: '运营人员',
  field_unconfirmed: '待审核',
  field_marster: '支持者',
  field_master: '支持者',
  field_external: '外部成员',
  field_project: '项目',
  field_create: '创建',
  field_order: '排序',
  field_delete: '删除',
  field_name: '名字',
  field_description: '摘要',
  field_type: '类型',
  field_cover: '封面',
  field_default_version: '默认版本',
  field_remove_version: '移除版本',
  field_chat_service: '通讯服务',
  field_invite_uris: '邀请加入',
  field_jsonContent: '主体内容',
  field_overview: '概览',
  field_media: '预览',
  field_start: '起始日期',
  field_end: '结束日期',
  field_deadline: '交付时间',
  field_board: '工作空间',
  field_group: '分组',
  field_status: '状态',
  field_icon: '图标',
  field_kanban: '看板',
  field_title: '标题',
  field_column: '分栏',
  field_card: '卡片',
  field_followed_bies: '关注',
  field_importance: '重要度',
  field_urgency: '紧急度',
  field_score: '分值',
  field_color_marker: '颜色标签',
  field_private: '设置私有',
  field_card_todogroups: '卡片待办分组',
  field_card_todo: '卡片待办',
  field_card_storage: '卡片存储',
  field_todogroups: '待办分组',
  field_todo: '待办',
  field_content: '内容',
  field_role: '角色',
  field_card_document: '卡片文档',
  field_sub_folders: '子文件夹',
  field_author: '作者',
  field_storage: '网盘',
  field_modify: '修改',
  field_read: '读取',
  field_manageMember: '管理成员',
  field_manageRole: '设置权限',
  field_create_version: '新增版本',
  field_files: '文件',
  field_mm_channel_id: '聊天频道',
  field_preferences: '偏好设置',
  field_document: '文档',
  field_schedule_event: '规划事件',
  field_schedule_events: '规划事件',
  field_subject: '事件内容',
  field_location: '地点',
  field_time: '时间',
  field_share: '分享',
  field_schedule: '规划',
  field_card_schedule: '任务规划',
  field_card_schedule_event: '任务规划事件',
  field_disable_share: '禁用共享',
  field_budget: '预算',
  field_create_ledger: '新增流水',
  field_delete_ledger: '删除流水',
  field_read_ledger: '查看流水',
  field_modify_ledger: '修改流水',
  field_approve_ledger: '批准流水',
  field_marker_todos: '标记待办',
  field_price: '服务价格',
  field_approve_join_request: '批准加入请求',

  role_name: '角色名称',
  permission_autoset_tip: '此处根据分类字段的配置权限自动设置',
  set_admin_only_tip: '仅限管理人员配置',
  set_no_permission_tip: '您无权设置此处内容',
  same_role_exit_tip: '该角色已存在',

  Initial_Version: '初始版本',
  初始版本: '初始版本',
  Initial_Column: '初始分栏',
  Initial_Kanban: '初始看板',
  Initial_Group: '初始分组',
  Workspace: '工作空间',
  Project_schedule: '项目规划',

  interact: '沟通',
  quick_post: '快报',
  toManyMode_team: '社群服务',
  toOneMode_team: '私域经营',
  channels_teamFunc: '频道功能',
  projects_teamFunc: '项目功能',
  disable_teamFunc_labe: '禁用功能',
  no_disable_teamFunc: '无禁用功能',
  team_toOne_mode_tip: '以向用户提供服务为目的，禁用社群相关功能',
  team_toMany_mode_tip: '以私域经营为目的，维护社群、提供服务、销售产品、项目协作',
  team_disableProjects_tip: '项目功能将不可用',
  team_disableChanel_tip: '频道功能将不可用',

  team_id_cant_lose: '团队ID参数不能缺少',
  initialization_step3_label_1: '运营模式：社群服务 VS 私域经营',
  initialization_step3_label_2: '讨论频道',
  initialization_step3_label_3: '项目管理',
  initialization_step3_label_4: '“对象型”讨论',
  initialization_step3_label_5: '数据共享与反馈收集',
  initialization_step3_label_6: '有机整合“团队”与“个人”',

  "town-square": '事务讨论',
  "off-topic": '闲聊区',

  userStatus_byTeam_blocked: '您已被当前团队屏蔽，无法查看团队内容',
  userStatus_byTeam_unconfirmed: '请等待管理人员的审核',
  userStatus_byTeam_tip: '您可以通过窗口左上角或右下角按钮切换或创建团队',
  
  onboarding_step1_title: '团队初始化',
  onboarding_step1_caption: '创建、或加入一个团队',
  onboarding_step2_title: '频道',
  onboarding_step2_caption: '了解频道，或新增频道',
  onboarding_step3_title: '30秒通关',
  onboarding_step3_caption: '快速了解平台功能',
  onboarding_completed_tip: '恭喜，初始化完成',
  onboarding_completed_btn_label: '开启团队之旅',

  team_deleted_tip_title: '团队已被删除',
  team_deleted_tip_caption: '请通过左上或左下角按钮切换或创建团队',

  memo: '备忘',
  tips: '提示',
  tips_unsupport_title: '以下文件的格式暂不支持：',
  tips_unsupport: '您可以将文件打包为压缩包后上传',
  affairs_filter: '事务筛选',

  class_overview: '概览',
  class_forum: '讨论',
  class_kanban: '延伸课',
  class_documents: '文档',
  class_storage: '文件',
  class_note: '笔记',

  multiple_versions: '多版本',
  only_electron: '仅限桌面端',
  quality_source: '原画',
  quality_3840: '4K',
  quality_1920: '全高清',
  quality_1280: '半高清',
  quality_960: '960P',
  quality_640: '640P',
  quality_480: '480P',
  quality_320: '320P',
  quality_240: '240P',
  quality_848: '848P',

  splite_kanban_tip: '分割视图',
  splite_kanban_right_view_tip: '请在右侧看板列表中点选看板',
  add_member_from_team: '从团队中添加',
  add_member_by_invite_link: '通过邀请链接添加',
  team_members_exceeded: '团队成员数量超额，请联系团队管理员升级付费计划',
  team_members_exceeded_tip: '由于成员数量超过限额，部分成员未能显示',

  send_friend_request: '发送好友请求',
  add_friend_msg_label: '留言',
  send_friend_dlog_title: '添加好友',

  version_updated_notify: '应用已更新，请使用“Shift + F5”或“Ctrl + Shift + r”重载应用。',
  delete_card_warning: '删除当前内容',

  create_person_todogroup: '创建个人事务分组',
  create_todogroup: '创建待办分组',
  budget: '预算',
  set_team_notification_label: '设置团队通知',
  confirm: '确认',
  reset: '重置',
  set_team_notification: '团队通知',
  add_team_news: '新增资讯',
  team_news_title: '资讯标题',
  add_team_news_confirm: '确认新增资讯',
  add_ledger: '新增流水',
  total_budget_amount: '总预算',
  budget_purpose: '预算用途',
  budget_table_createAt: '创建时间',
  budget_table_amount: '金额',
  budget_table_status: '状态',
  budget_table_purpose: '说明',
  budget_table_handler: '经办人',
  budget_table_authorizer: '授权人',
  budget_table_more: '更多',
  budget_table_balance: '余额',
  budget_table_balance_tip: '(不含未批准)',
  budget_table_approved: '已批准',
  budget_table_approveIt: '批准',
  budget_add_budget: '添加预算',
  budget_amount: '预算金额',
  ledger_outcome: '支出',
  ledger_income: '收入',
  ledger_amount: '用款金额',
  ledger_purpose: '说明款项用途',
  ledger_update_amount: '调整金额',
  ledger_delete: '删除该流水',
  ledger_affect_budget: '调整预算',

  document_locked_tip: '其它用户正在编辑该文档',
  read_only_mode: '只读查看',
  document_unlock: '解锁',
  document_unlock_tip: '请确保：文档的锁定是因为意外原因导致的，否则修改内容可能导致其它人的编辑内容丢失',
  no_tasks: '暂无待办任务',

  create_notebook: '创建笔记本',
  create_note: '创建笔记',
  open_related_course: '打开关联课程',
  notebooks: '笔记',
  app_notebooks_purpose: '链接笔记',

  start_meet: '发起会议',
  join_meet: '加入会议',
  meet_no_auth: '您无权参与此会议',


  tiptap_menu_Text: '文本',
  tiptap_menu_Text_description: '直接开始输入纯文本。',
  tiptap_menu_TodoList: '待办列表',
  tiptap_menu_TodoList_description: '使用待办列表跟踪任务。',
  tiptap_menu_H1: '标题1',
  tiptap_menu_H1_description: '大节标题。',
  tiptap_menu_H2: '标题2',
  tiptap_menu_H2_description: '中节标题。',
  tiptap_menu_H3: '标题3',
  tiptap_menu_H3_description: '小节标题。',
  tiptap_menu_BulletList: '无序列表',
  tiptap_menu_BulletList_description: '创建简单的无序列表。',
  tiptap_menu_NumberedList: '有序列表',
  tiptap_menu_NumberedList_description: '创建带编号的列表。',
  tiptap_menu_Quote: '引用',
  tiptap_menu_Quote_description: '捕获引用。',
  tiptap_menu_Code: '代码',
  tiptap_menu_Code_description: '捕获代码片段。',
  tiptap_menu_Image: '图片',
  tiptap_menu_Image_description: '从计算机上传图片。',
  tiptap_slash_menu_text: '段落',
  tiptap_slash_menu_list: '列表',
  tiptap_slash_menu_block: '区块',
  tiptap_slash_menu_insert: '插入',

  buy: '购买',
  buied: '已购',
  my_publish: '我发布的',
  buy_service: '购买服务',
  buy_completed: '购买完成',
  favorite: '收藏',
  liked: '点赞',
  tutorial: '教程',
  purchased: '已购',
  home: '首页',
  drop_or_pick_cover: '拖拽或点击选择封面',
  drop_or_pick_preview: '拖拽或点击选择预览',
  drop_or_pick_id_card: '拖拽或点击选择身份证',
  drop_or_pick_id_card_back: '拖拽或点击选择身份证背面',
  drop_or_pick_resource: '拖拽或点击选择资源',
  drop_or_pick_deliver_file: '拖拽或点击选择交付文件',
  drop_or_pick_work_cover: '拖拽或点击选择作品封面',
  drop_or_pick_work_media: '拖拽或点击选择作品媒体',
  support_type: '支持类型',
  max_file_size: '最大文件大小',
  drop_or_pick_logo: '拖拽或点击选择Logo',
  selected_this_auth_type: '选择此授权类型',
  price_cannot_be_negative: '价格不能为负数',
  add_version_completed: '版本已创建',
  unpublished: '未发布',
  publish: '发布',
  pulled: '下架',
  unpulled: '上架',
  is_pulled: '已下架',
  price: '价格',
  price_free: '免费',
  unsupported_file_format: '不支持此格式文件',
  app_notebook_title: '我的笔记',
  self_published: '我发布的',
  paied: '已购',
  pulled_card_tip: '下架后内容将不可见，不影响以购买者访问，下架内容不再消耗项目存储空间！',
  cant_published_no_media_tip: '没有包含任何内容，不能发布',

  works: '作品',
  cv: '简历',

  logo_section: '标志',
  name_section: '名称',
  mode_section: '模式',
  funcition_section: '功能开关',

  storage_capactiy_exceeded: '团队文件容量超限，请联系管理员升级订阅',
  storageCapacityExceeded_notify_title: '订阅超限',
  storageCapacityExceeded_notify_message: '团队文件容量超限，请联系管理员升级订阅',
  cardNumberExceeded_notify_title: '订阅超限',
  cardNumberExceeded_notify_message: '团队卡片数量容量超限，请联系管理员升级订阅',

  my_business: '经营中心',
  business_overview: '经营概览',
  deal_page: '协作',
  business_account_info: '账号信息',
  business_certificate_info: '认证信息',
  total_revenue: '总收入',
  account_balance: '账户余额',
  total_revenue_description: '您在平台销售金额 - 支付平台服务费用 - 平台佣金',
  account_balance_description: '总收入 - 已提现金额',
  withdraw: '提现',
  
  known_more: '了解更多',
  init_known_more: '请逐个点击下方内容查看后完成初始化',

  next_step: '下一步',
  previous_step: '上一步',
  onboarding_skip_btn_label: '跳过初始化',
  read_functions_tip: '点击并阅读上方功能介绍，以便快速了解平台功能',

  need_fill_business_account: '您尚未完善经营账号信息！',
  fill_business_account: '完善信息',

  join_request: '申请加入',
  create_join_request: '请求加入',
  create_join_request_success: '请求已发送，请等待管理员审核',
  join_request_message: '申请留言',
  no_join_request: '暂无来自团队成员的加入申请',
  accept: '接受',
  reject: '拒绝',
  show_in_discover: '在“发现”中显示',

  get_more: '获取更多',
  no_more: '没有更多了',
  discover: '发现',
  app_discover_purpose: '查看、购买、管理资产或服务',

  file_too_large: '文件大小不能超过 {size}MB',
  exclusive: '独占授权',
  commercial: '商用授权',
  'non-commercia': '非商用授权',
  educational: '教育授权',
  personal: '个人授权',
  price_limit: '最低价格： 按文件大小，最低1元 / 每GB',
  clear_topic: '清空会话',
  ai_assistant: 'AI智能体',
  ai_topics: '会话',
  default_assistant: '默认智能体',
  add_assistant: '新增智能体',
  assistant_name: '智能体名称',
  assistant_prompt: '智能体提示词',
  enter_topic: '进入会话',
  settings_for: '设置',
  assistant_name: '智能体名称',
  assistant_prompt: '智能体提示词',
  add: '添加',
};
